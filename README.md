...
运行期可以通过 `GET /config` 查询当前配置，通过 `PATCH /config` 更新 `sources.multiSourceEnabled` 和下载相关策略。下载任务创建时不再接收音质/音源策略参数，而是直接使用当前配置快照。
...
| 指定源/全源下载策略 | `GET/PATCH /config`, `POST /downloads` | `get_config`, `update_config`, `create_download_task` | 部分实现 | 部分验证 | 下载策略只能通过配置文件、环境变量或配置 API 设置；任务创建时使用当前配置快照。已验证 HTTP/MCP 创建任务、本地 `file:` 下载完成和 `sourceStrategy: all` 入任务；真实音乐文件下载未验证。 |
| 下载品质策略 | `GET/PATCH /config`, `POST /downloads` | `get_config`, `update_config`, `create_download_task` | 已实现 | 部分验证 | 支持 `qualityStrategy: specified/highest/lowest`，由配置驱动；已验证配置下发到任务，最高/最低真实 URL 选择仍需补充验证。 |
...
| 歌词获取 | `POST /lyrics/get` | `get_lyric` | 已实现 | 部分验证 | 内置源优先使用 `smart-lyric` 支持的歌词下载/解析能力，失败后回退到平台协议实现；支持普通、翻译、罗马音和逐字歌词字段，咪咕使用 MRC/LRC/TRC 回退实现。 |
...
| 专辑详情 | `POST /albums/detail` | `get_album_detail` | 部分实现 | 部分验证 | 已验证 LX 用户源 `album` action 转发；真实平台详情数据未完整验证。 |
| 歌手详情 | `POST /singers/detail` | `get_singer_detail` | 部分实现 | 部分验证 | 已验证 LX 用户源 `singer` action 转发；真实平台详情数据未完整验证。 |
| 歌曲详情 | `POST /music/detail` | `get_music_detail` | 部分实现 | 部分验证 | 已验证 LX 用户源 `musicDetail` action 转发；真实平台详情数据未完整验证。 |
...
- 按功能拆分测试文件：`test/sources.test.js`、`test/lx-protocol.test.js`、`test/music.test.js`、`test/downloads.test.js`、`test/docs.test.js`、`test/metadata.test.js`、`test/health.test.js`。
- 输出真实音源支持的平台和音质。
- 使用真实歌曲名 `紅蓮華` 分别搜索 `kg/kw/mg/tx/wy`。
- 验证 HTTP handler 的健康检查、运行配置、音源列表/详情/启用/禁用/重载/升级提示、搜索、匹配、合并后的音乐 URL、下载任务、歌词、封面、元数据、API 文档和不支持能力业务码。
- 验证 MCP handler 的 `tools/list`、运行配置、音源查询/列表/更新/删除/重载/升级检查、搜索、音乐 URL、多音质 URL、创建下载任务、下载任务详情。
