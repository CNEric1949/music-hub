...
源注册表和下载任务状态默认写入 `data/music-hub.sqlite`。音乐源导入入口包括启动时扫描 `data/sources/*.js`、用户上传文件导入和 URL 在线导入；脚本文件仍保存在 `data/sources/*.js`。下载任务启动时兼容迁移旧 `data/download-tasks.json`。
