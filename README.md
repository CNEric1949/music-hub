...
- [ ] MCP 音源启用/禁用 Tool。
- [ ] 搜索失败隔离测试，构造失败源验证 `failures` 返回。
- [ ] 咪咕 `紅蓮華` LiSA 版本歌词补验。
- [ ] 独立歌词保存接口测试，以及翻译/罗马音/LX 歌词合并细节覆盖。
- [ ] 下载后真实封面处理端到端任务测试。
- [ ] 下载任务完成后歌词、封面与音频标签嵌入的真实平台端到端验证。
- [ ] 专辑、歌手、歌曲详情真实平台数据完整验证。
- [ ] HTTP/MCP 端口监听型端到端测试。
- [ ] Dockerfile/compose 部署适配与验证。
- [ ] 咪咕封面搜索结果图片字段优先级确认。
...
| 下载后歌词/封面处理 | 下载任务配置 | 无 | 部分实现 | 部分验证 | 下载完成后可保存歌词、各类歌词变体、尝试下载封面并写入音频标签；当前自动化验证本地 `file:` 下载和歌词文件落盘，真实平台下载任务后的封面与标签嵌入仍待端到端覆盖。 |
